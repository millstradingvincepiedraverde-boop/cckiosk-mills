...
export default function AgentPage() {
    ...
    const attemptPlay = async (video?: HTMLVideoElement | null) => {
        ...
        try {
            ...
            // ensure element is visible (some mobile browsers need display:block)
            (video.style as any).display = (video.style as any).display || "block";
    ...
    const handleJoinRoom = async (roomId: string) => {
        ...
        try {
            ...
            // prepare persistent remote stream
            if (!persistentRemoteRef.current) persistentRemoteRef.current = new MediaStream();
            if (remoteVideoRef.current && !remoteVideoRef.current.srcObject) remoteVideoRef.current.srcObject = persistentRemoteRef.current;
            ...
            pc.onconnectionstatechange = () => {
                ...
                if (pc.connectionState === "connected") {
                    ...
                    setJoining(false);
                    setStatus("Connected!");
                    // re-attempt play (in case blocked earlier)
            ...
            // sdp filter to prefer H264/VP8 for iOS
            ...
            // joinRoom should: set remote description (offer), write answer, and add remote ICE candidates to pc
            await joinRoom(roomId, pc, sdpFilter);

            setStatus("Waiting for remote media...");
            setJoining(false);

            // try play in case tracks already attached
            attemptPlay(remoteVideoRef.current).catch(() => { });
        } catch (err) {
